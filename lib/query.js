...
async function viewEmployees(){ 
    try{
    const response = await pQuery("SELECT e.id, e.first_name, e.last_name, role.title as role, concat(e2.first_name, ' ', e2.last_name) as manager FROM employee e INNER JOIN role ON e.role_id = role.id LEFT JOIN employee e2 ON e.manager_id = e2.id");
...
module.exports = {init, viewEmployees, viewDepartments, viewRoles, viewEmployeesByManager, addEmployee, addRole, addDepartment, updateRole, endQuery};